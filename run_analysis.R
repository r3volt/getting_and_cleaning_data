...
createDataSet<-function(dataDirectory="./data", dataSetName="train") {
  ...
  ## This is using eval and parse to make more modular.
...
#   cmd <- paste('df <- cbind(subject_', dataSetName, ', y_', dataSetName, ')', sep="")  
...
#   dataNames<-c("subject", "X", "y")
#   fileName<-paste(dataNames[2], "_", setNames[1], ".txt", sep="")
#   x_trainPath<-paste(mergeDataDirectory, sourceDataDirectoryName, setNames[1], fileName, sep="/")
# 
#   subject_train <- read.table(filePath, widths = lineFixedWidthDefinition, header = FALSE)
#   X_train<-read.table(filePath, widths = lineFixedWidthDefinition, header = FALSE)
#   
# 
}
...
trimmed<-select(full, contains("mean"), contains("std"))
